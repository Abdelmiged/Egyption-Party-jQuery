...
// Initialize accordion states and timer countdown
...
$(".details__singer h2").on("click", function() {
    ...
    $(".details__singer p").not($(this).next()).each(function() {
        $(this).slideUp(500);
    })
})
...
$(".contact__message-field").on("input", function() {
    ...
    if(numberOfCharacters > 0 && ($(this).val().length > oldMessage.length)) {
        $(".characters-remaining").text(String(numberOfCharacters - ($(this).val().length - oldMessage.length)));
        oldMessage = $(this).val();
    }
    else {
        $(".characters-remaining").text(String(numberOfCharacters + (oldMessage.length - $(this).val().length)));
        oldMessage = $(this).val();
    }
})